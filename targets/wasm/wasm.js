...
function Wasm32VM(sizes)
{
    ...
    this.shadowtable = {};
    this.sizes = sizes;
    this.memory = new WebAssembly.Memory({initial: 4228, maximum: 4228});
...
Wasm32Module.prototype.instantiate = function (thread, vm, mem, recurse)
{
    ...
    this.imports.sys.read_file = thread.read_file.bind(thread);
    this.imports.sys.open_file = thread.open_file.bind(thread);
    this.imports.sys.emit = thread.emit_char.bind(thread);
    this.imports.sys.bye = thread.bye.bind(thread);
...
Wasm32Thread.prototype.read_file = function (fileid, u1, addr)
{
    var i;

    if (fileid === 0 && (!fhs[fileid] || this.vm.HEAPU8[fhs[fileid].offset + 32] === 0)) {
       fhs[0] = { offset: 1023 * 1024 };
       for (var i = 0; i < 1024; i++)
           this.vm.HEAPU8[1023 * 1024 + i] = 0;
       var str;
       do {
           str = readline();
       } while (str === "");
       if (!str)
          foreign_bye(0);
       var len = CStringTo(str, this.vm.HEAPU8, fhs[0].offset + 32);
       this.vm.HEAPU8[1024 * 1023 + 32 + len - 1] = "\n".charCodeAt(0);
       this.vm.HEAPU8[1024 * 1023 + 32 + len] = 0;
    }
    var off = fhs[fileid].offset;

    for (i = 0; i < u1; i++)
        if ((this.vm.HEAPU8[addr++] = this.vm.HEAPU8[fileid + off + 32 + i]) == 0)
           break;
...
Wasm32Thread.prototype.load_file = function (heapu8, path)
{
    var str;
    try {
    ...
    this.vm.HEAPU32[next_load_address+4>>2] = 0; // position
    this.vm.HEAPU32[next_load_address+8>>2] = load_size[path]-1; // size
    this.vm.HEAPU32[next_load_address+12>>2] = 1; // call slow_read flag
    next_load_address += 32 + load_size[path];
}

Wasm32Thread.prototype.load_files = function (heapu8)
{
    this.load_file(heapu8, "src/load.fth");
    this.load_file(heapu8, "src/core.fth");
    this.load_file(heapu8, "src/core-ext.fth");
    this.load_file(heapu8, "src/string.fth");
    this.load_file(heapu8, "src/tools.fth");
    this.load_file(heapu8, "src/file.fth");

    this.load_file(heapu8, "targets/asmjs/meta.fth");
    this.load_file(heapu8, "targets/asmjs/params.fth");
    this.load_file(heapu8, "threading.fth");
    this.load_file(heapu8, "params.fth");
    this.load_file(heapu8, "lib/meta.fth");
    this.load_file(heapu8, "src/search.fth");
    this.load_file(heapu8, "lib/image.fth");
    this.load_file(heapu8, "lib/cross.fth");
    this.load_file(heapu8, "targets/asmjs/target.fth");
    this.load_file(heapu8, "targets/asmjs/asm.fth");
    this.load_file(heapu8, "lib/xforward.fth");
    this.load_file(heapu8, "targets/asmjs/nucleus.fth");
    this.load_file(heapu8, "targets/asmjs/next.fth");
    this.load_file(heapu8, "targets/asmjs/jump.fth");
    this.load_file(heapu8, "src/kernel.fth");
    this.load_file(heapu8, "src/dictionary.fth");
    this.load_file(heapu8, "jump.fth");
    this.load_file(heapu8, "src/file.fth");
    this.load_file(heapu8, "targets/asmjs/cold.fth");
    this.load_file(heapu8, "target.fth");
    this.load_file(heapu8, "targets/asmjs/dump.fth");
}

Wasm32Thread.prototype.open_file = function (mode, u, addr, u2)
{
    console.log(mode + " " + u + " " + addr + " " + u2)
    var path = StringAt(this.vm.HEAPU8, addr, u);
    var mode = CStringAt(this.vm.HEAPU8, mode);

    var fileid = 0;

    if (!(path in load_address))
        this.load_file(this.vm.HEAPU8, path);

    if (path in load_address) {
        fileid = load_address[path];
        fhs[fileid] = { offset: 0 };
        this.vm.HEAPU32[load_address[path]+4>>2] = 0; // reset position.
    }

    console.log("loading " + path + ": " + fileid)
...
Wasm32Thread.prototype.emit_char = function (a, u, x, y)
{
    foreign_putchar(a);
    return 127;
}

Wasm32Thread.prototype.bye = function (a, u)
{
    console.log("bye " + a + " " + u);
}
...
Wasm32Thread.prototype.step = function ()
{
    ...
    while (true) {
        if (first) {
            var tk = find_words(this.vm.HEAPU32, this.vm.HEAPU8);
            try {
            ret = system.threads[0].exports.forth(tk, 0, 512 * 1024, 510 * 1024);
            } catch (e) {
                console.log(e);
            var tk = find_words(this.vm.HEAPU32, this.vm.HEAPU8);
            }
...
function print_words(word, HEAPU32, HEAPU8)
{
    var name = StringAt(HEAPU8, word+1, HEAPU8[word] > 128 ? 256-HEAPU8[word] : HEAPU8[word]);
    var next = HEAPU32[word+16>>2];

    console.log(name + " @ " + word.toString(16) + " " + HEAPU32[word+28>>2].toString(16));
    words[name] = word;
    if (next)
    print_words(next, HEAPU32, HEAPU8);
}

function find_words(HEAPU32, HEAPU8) {
    var word;
    for (word = 16384; HEAPU32[word>>2] == 0; word -= 4);
    word -= 32;
    print_words(word, HEAPU32, HEAPU8)

    HEAPU32[words["latest0"]+28>>2] = words["turnkey"];
    HEAPU32[words["sp0"]+28>>2] = 512 * 1024;
    HEAPU32[words["rp0"]+28>>2] = 510 * 1024;
    HEAPU32[words["dp0"]+28>>2] = 513 * 1024;

    return words["turnkey"]
}

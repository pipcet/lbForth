...
{
    var tmp = [this.dom_.style.background, this.dom_.style.color];
    //console.log("tmp " + tmp);
    tmp[0] = tmp[0].replace(/ .*/, "");
    tmp[1] = tmp[1].replace(/ .*/, "");
...
function VT100FD(process, domparent, fdno = undefined, outfdno = undefined)
{
    ...
    if (true) {
        this.outfd = {};
        if (1) document.addEventListener("keypress", e => {
            if (this.domparent.contains(e.target) ||
                ...
                window.setTimeout(function () {
                    ...
                    console.log("input string (2) " + inputstr);
                }, 0);
            }
        });
    }
...
VT100FD.prototype.outputPromise = function (heap, ptr, len)
{
    ...
    for (i=0; i<len; i++) {
        ...
        case "escape":
            ...
            } else if (m = this.escapeSequence.match(/^\[(\d*)[fH]$/)) {
                this.context.x = 0;
                this.context.y = 0;
                if (m[1] !== "")
                    this.context.y = m[1] - 1;
            ...
            } else if (m = this.escapeSequence.match(/^\[(\d*)J$/)) {
                var x, y;
                for (y = this.y; y < 25; y++) {
                    for (x = 0; x < 80; x++)
...
function putstr(c)
{
    var ab = new ArrayBuffer(c.length+1);
    CStringTo(c, ab, 0);
    vt100.outputPromise(ab, 0, c.length);
    vt100.triggerRefresh();
}

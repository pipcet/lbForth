...
: colon-runtime   c" docolcomma4_ip" ;
...
: .code1   ." function " latestxt >name .mangled
   ." _code (IP, word)" cr s"     return IP;" latestxt ;
...
: .ref   ?dup if [char] " emit >name .mangled ." _word" [char] " emit else ." 0" then ;
: .ref0 ." [" .ref ." , 0]" ;
...
: .does   >does @ .addr ., ;
: .cr   cr ;
: .(literal)   ['] (literal) .ref0 ., .cr ;
: .branch ( a xt -- u )   .ref0 ., .cr @ .addr 2 cells ;
: .literal ( a xt -- u )   .ref0 ., .cr dup addr? if @ .addr
   else @ (.) then 2 cells ;
...
: .;   ." ;" cr ;
: .winit ." var " >name .mangled ." _word" .; ;
: .wcreate >name .mangled ." _word = new Word()" .; ;
: .wname dup >name .mangled ." _word.prelink.name = " >name .quoted .; ;
: .wnlen dup >name .mangled ." _word.prelink.nlen = " c@ (.) .; ;
: .wmname dup >name .mangled ." _word.prelink.mname = " [char] " emit >name .mangled [char] " emit .; ;
: .wlink dup >name .mangled ." _word.prelink.next = " >nextxt .ref .; ;
: .wdoes dup >name .mangled ." _word.prelink.does = " >does @ .addr .; ;
: .wcode dup >name .mangled ." _word.prelink.code = " >code @ dup t-xt? if >name .mangled ." _code" else drop ." 0" then .; ;
: .wparam0 >name .mangled ." _word.prelink.param = [" ;
: .wparam1 drop ." ]" .; ;
: .wbody body-bounds ?do i @+ .cell +loop ;
: .wword dup .wcreate dup .wname dup .wmname dup .wnlen dup .wlink dup .wdoes dup .wcode dup .wparam0 dup .wbody .wparam1 ;
: dtd-0 0 begin >prevxt dup .winit dup latestxt = until drop ;
: dtd-1 0 begin >prevxt dup .wword dup latestxt = until drop ;
...
interpreter-context
s" docol," .extern
include targets/js/nucleus.fth
include kernel.fth
